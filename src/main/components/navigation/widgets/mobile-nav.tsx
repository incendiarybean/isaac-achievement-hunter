...
const MobileNav = ({ steamDetails, setSteamDetails }: NavigationComponent) => {
    ...
    return (
        <div className="ml-2 -my-1 sm:hidden">
            ...
            <div hidden={!settingsOpen} className="animate-fadeIn absolute top-0 left-0 w-full h-screen bg-zinc-900 bg-opacity-70">
                <div className="flex justify-end w-full h-full">
                    <div ref={accountElement} className="bg-zinc-900 shadow w-3/4 p-2 flex flex-col">
                        <div className="flex justify-between items-center pt-2 pb-1 border-b border-zinc-800 mr-2">
                            <h1 className="uppercase leading-wide font-bold">Menu</h1>
                            <button onClick={() => setSettingsOpen(false)} className="text-red-500 hover:text-red-700">
                                <CrossIcon />
                            </button>
                        </div>
                        <button
