...
const TableRowDetail = ({ currentPageData, url }: TableWidgets) => {
    return (
        <>
            {currentPageData.map((data: Achievement) => (
                <div className="group" key={`${data.displayName}-pagination-${data.name}-${new Date().toISOString()}`}>
                    <button
                    ...
                    >
                        ...
                        <p className="flex-auto sm:flex-none h-full w-24 md:w-56 p-2">{data.displayName}</p>
                        <p className="hidden sm:block flex-auto h-full p-2 overflow-auto">{data.helper}</p>
                        <p
                            className={`${
                                !Object.keys(data).includes("achieved") && "hidden"
